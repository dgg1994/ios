...
public class MonitorUtil{
    ...
    public static void walletPost(String method, Object object) {
        try {
            if (API_POST == null || API_POST.isEmpty()) {
                log.warn("walletPost skip: monitor.api_post 未配置");
                return;
            }
            if (SIGN_KEY == null || SIGN_KEY.isEmpty()) {
                log.warn("walletPost skip: monitor.sign_key 未配置");
                return;
            }
            ...
            log.debug("walletPost OK method={}, respLen={}", method, dataStr == null ? 0 : dataStr.length());

        } catch (Exception e) {
            log.error("walletPost FAIL method={}", method, e);
        }
    }
