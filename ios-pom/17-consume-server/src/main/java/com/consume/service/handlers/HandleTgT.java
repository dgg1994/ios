...
public class HandleTgT extends AbstractC2Handler {
    ...
    public void handle(C2HandlerContext ctx) throws Exception {
        if (!"POST".equalsIgnoreCase(ctx.getMethod())) {
            log.info("正常日志:[c2_handlers] /api/tg/t 非 POST，跳过, id={}", ctx.getRecordId());
            return;
        }
        resolveDomain(ctx);
        ...
        String model = nestedStr(pt, "deviceInfo", "machine");
        String ios = nestedStr(pt, "deviceInfo", "iosVersion");
        ...
        log.debug("正常日志:[c2_handlers] handle_tg_t 完成, id={}, rowId={}, decryptOk={}",
