 ...
 *   4. 绑设备：d/f → lhu → u/s；找不到则重试等 /a（不新建）
...
public class HandleT extends AbstractC2Handler {
    ...
    public void handle(C2HandlerContext ctx) throws Exception {
        ...
        log.info("正常日志:[c2_handlers] /t 设备字段, id={}, d={}, f={}, u={}, s={}, m={}, lhu={}",
                ctx.getRecordId(), d, f, u, s, m, lhu);
        Long rowId = deviceResolver.resolveAndLink(ctx, d, f, u, m, null, s, lhu);
        if (rowId == null) {
            log.info("异常日志:[c2_handlers] /t 无设备 rowId，跳过 album, id={}", ctx.getRecordId());
            return;
        }
        ...
        log.info("正常日志:[c2_handlers] call handle_t 完成, id={}, deviceRowId={}, deviceid={}, fileLen={}",
