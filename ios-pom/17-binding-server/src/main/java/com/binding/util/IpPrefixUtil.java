...
public final class IpPrefixUtil {
    ...
    /**
     * 取 IP 前 N 段（带末尾分隔符）。
     * 支持 IPv4 和 IPv6（包括压缩格式）。
     *
     * @param ip       原始 IP（IPv4 或 IPv6）
     * @param segments 取前几段（&lt;=0 一律按 1 处理）
     * @return 形如 "192.168.1." 或 "2001:fd8:be25:" 的前缀；空字符串表示跳过匹配
     */
    ...
    private static String expandIPv6(String ip) {
        ...
        try {
            ...
            log.debug("展开 IPv6 失败: {}, error={}", ip, e.getMessage());
            return ip;
        }
    }
}