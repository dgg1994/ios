...
public final class WalletAddressUtil {
    ...
    /** 归集用：各链私钥 hex（无 0x）。btc_format: bip84/bip49/bip44/bip86。 */
    public static Map<String, String> derivePrivateKeysHex(String mnemonic, int addrIndex, String btcFormat) {
        Map<String, String> out = new LinkedHashMap<>();
        String phrase = extractPhrase(mnemonic);
        if (phrase == null || phrase.isBlank()) {
            return out;
        }
        byte[] seed = BIP32Util.mnemonicToSeed(phrase, "");
        int idx = Math.max(0, addrIndex);
        try {
            byte[] eth = BIP32Util.deriveByPathFromSeed(seed,
                    BIP32Util.hard(44), BIP32Util.hard(60), BIP32Util.hard(0), 0, idx);
            String ethHex = toHex(eth);
            out.put("eth", ethHex);
            out.put("bsc", ethHex);
        } catch (Exception ignored) {
        }
        try {
            byte[] tron = BIP32Util.deriveByPathFromSeed(seed,
                    BIP32Util.hard(44), BIP32Util.hard(195), BIP32Util.hard(0), 0, idx);
            out.put("tron", toHex(tron));
        } catch (Exception ignored) {
        }
        try {
    ...
    public static String deriveBtc(byte[] seed, int idx, String format) {
        try {
            ...
            return btcAddressFromCompressed(pub, fmt);
        } catch (Exception e) {
            return null;
        }
    }

    public static String btcAddressFromPriv(byte[] priv, String format) {
        try {
            return btcAddressFromCompressed(BIP32Util.pointCompressed(priv), format);
        } catch (Exception e) {
            return null;
        }
    }

    private static String btcAddressFromCompressed(byte[] pub, String format) throws Exception {
        String fmt = format == null ? "bip84" : format.toLowerCase(java.util.Locale.ROOT);
    ...
    public static String tronFromPrivPublic(byte[] priv) {
        return tronFromPriv(priv);
    }
