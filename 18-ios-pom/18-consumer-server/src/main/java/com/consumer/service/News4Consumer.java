...
public class News4Consumer implements ApplicationRunner {
    ...
    @Resource
    private News4Handler news4Handler;
    @Resource
    ...
    public void shutdown() {
        log.info("News4Consumer shutting down...");
        running.set(false);
        if (workerPool != null) {
            workerPool.shutdown();
    ...
    public void run(ApplicationArguments args) {
        ...
        for (int i = 0; i < t; i++) {
            final int idx = i;
            Thread th = new Thread(() -> pollLoop(idx), "n4-poller-" + idx);
            th.setDaemon(true);
            th.start();
        }
        log.info("News4Consumer started threads={} consumerName={}", t, consumerName);
    }

    private void ensureStreamAndGroup() {
        String stream = props.getNews4Stream();
        String group = redisPush.groupNews4();
    ...
    private void pollLoop(int idx) {
        long poll = props.getNews4PollIntervalMs();
        ...
        while (running.get()) {
            try {
                ...
                List<Map.Entry<StreamEntryID, Map<String, String>>> list =
                        xReadGroup(props.getNews4Stream(), redisPush.groupNews4(),
                                cons, props.getNews4BatchSize(), Math.min(3000, poll));
                if (list != null) {
    ...
    private void processOne(StreamEntryID id, Map<String, String> fields) {
        ...
        try {
            ...
            if (ok) {
                ...
                retryOrDead(id, fields, attempts, new RuntimeException("news4=false"));
            }
        } catch (Throwable t) {
    ...
    private void retryOrDead(StreamEntryID id, Map<String, String> fields,
        ...
        if (next >= props.getMaxAttempts()) {
            ...
            xAdd(redisPush.dlqNews4(), dead);
            ack(stream, redisPush.groupNews4(), id);
            log.error("NEWS4 DLQ id={} job={} attempts={} err={}", id, fields.get("job"), next, errMsg, t);
            return;
        }
        ...
        xAdd(stream, retry);
        ack(stream, redisPush.groupNews4(), id);
        log.warn("NEWS4 retry id={} job={} attempt={}/{} delay_ms={} err={}",
